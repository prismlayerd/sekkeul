...
CAPTIONS = {
    ...
    # 5번은 2026-08-24에 양식 목록으로 바꿨다. 설정 화면은 「아무도 이걸 보고
    # 설치하지 않는다」에 걸려서 뺐다 — 프라이버시는 1번 홈 푸터의
    # 「이 명세서는 기기 안에만 있습니다」가 대신 말한다.
    5: ('연말정산·종소세·경정청구', '양식 19종을 담았어요'),
    6: ('연봉 실수령액부터 퇴직금까지', '계산기 56종'),
    7: ('몰라서 못 받는 정부 혜택', '54가지를 모았어요'),
    8: ('직장인 · N잡러 · 프리랜서', '고르면 화면이 바뀌어요'),
    # 뺀 프라이버시 컷. 다시 쓰려면 파일 이름을 09로.
    9: ('계정도 서버도 없어요', '지우면 정말로 사라져요'),
}
