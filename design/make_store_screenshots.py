...
CAPTIONS = {
    ...
    # 5번은 프라이버시 컷이다. 설정 화면의 「데이터 수집 · 수동 입력」과 붉은
    # 「개인 세무 데이터 영구 파기」가 등록정보의 첫 절을 그대로 증명한다.
    # 문구는 **주장이 아니라 사실**만 쓴다 — 릴리스 매니페스트에 INTERNET
    # 권한이 없어서 보낼 곳이 없는 게 맞다.
    5: ('계산도 저장도', '이 휴대폰 안에서만 해요'),
    6: ('연봉 실수령액부터 퇴직금까지', '계산기 56종'),
    7: ('몰라서 못 받는 정부 혜택', '54가지를 모았어요'),
    8: ('직장인 · N잡러 · 프리랜서', '고르면 화면이 바뀌어요'),
    9: ('연말정산·종소세·경정청구', '양식 19종을 담았어요'),
}
