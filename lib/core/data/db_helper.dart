...
class SqfliteDatabaseHelper implements DatabaseService {
  ...
  Future<void> _onUpgrade(Database db, int oldVersion, int newVersion) async {
        ...
        // 건강보험 지역가입자 연간 납부액 (v38).
        //
        // **지금은 아무 데서도 안 쓴다.** 만들 때는 전액 소득공제 대상이라고
        // 보고 넣었는데, 2026-07-25 확인에서 뒤집혔다 — 지역가입자 본인
        // 건보료는 필요경비 산입 대상이 아니고(국세청 서면인터넷방문상담1팀-998),
        // 소득세법 §52 특별소득공제는 근로소득금액 범위 안에서만 되므로
        // (같은 팀-476) 근로소득이 없는 프리랜서는 어느 쪽으로도 못 받는다.
        // 근거는 freelancer_tax.dart에 있다.
        //
        // 칸은 남긴다 — 지우려면 또 이전을 해야 하고, 비어 있는 칸은 해가 없다.
        if (oldVersion < 38) {
