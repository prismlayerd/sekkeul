...
class YearCoverage {
  ...
  /// 공제율이 다른 세 갈래의 **1월~지난달 누적액**.
  ///
  /// 이번 달부터는 가계부가 직접 받는다(`ExpenseItem.deductionType`) — 하루
  /// 입력의 「공제 구분」 칸이다. 여기 담기는 것은 앱을 깔기 **전** 기간뿐이고,
  /// 그때 것은 날짜별로 쪼갤 수가 없어(없는 기록을 지어내는 셈이라) 합계로 둔다.
  ///
  /// 두 곳이 겹치지 않는 이유: 백필 화면은 「1~지난달」만 묻고, 가계부 표식은
  /// 실제 기록에만 붙는다. 홈에서 둘을 더해 쓴다.
  ///
